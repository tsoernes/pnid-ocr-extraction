...
public class ImageFactory_SVG implements GraphicFactory {
	 ...
	 */
	@Override
	public void addLine(Color c, float lineWeight, LineElement.LineType lineType, double[] xCoordinates, double[] yCoordinates) {
		...
		polyline.setAttributeNS(null, "stroke-dasharray", lineType.getSvgStyle((lineWeight * this.s)));
	 ...
	 */
	@Override
	public void addShape(Color c, float lineWeight, LineElement.LineType lineType, double xCoordinates[], double yCoordinates[], boolean filled) {
		...
		polygon.setAttributeNS(null, "stroke-dasharray", lineType.getSvgStyle((lineWeight * this.s)));
	 ...
	 */
	@Override
	public void addEllipse(Color c, float lineWeight, LineElement.LineType lineType, double[] position, double primAxis, double secAxis,
		...
		ellipse.setAttributeNS(null, "stroke-dasharray", lineType.getSvgStyle((lineWeight * this.s)));
	...
	private String getPathArc(double[] center, double start, double end, double radius) {
		...
		if ((int) degree == 360) {
			...
			return this.getPartialCirclePath(points, radius, (degree < 180 || degree == 180 && start == 0) ? 1 : 0);
		}
	}
	 ...
	 */
	@Override
	public void addCircle(Color c, float lineWeight, LineElement.LineType lineType, double[] position, double radius, double startAngle,
		...
		circle.setAttributeNS(null, "stroke-dasharray", lineType.getSvgStyle((lineWeight * this.s)));
...
*            the height of the text
	...
	public void addText(Color c, double[] position, double[] extent, double textAngle, String string, double height,
		...
		final double fontSize = height * FONT_SIZE_FACTOR * this.x;
		...
		text.setAttributeNS(null, "font-size", df.format(fontSize));
		...
		// split the text via the linebreaks and draw each substring in another tspan
		String[] subStrings = linebreakPattern.split(string);
		for (int i = 0; i < subStrings.length; ++i) {
			Element tspan = this.doc.createElementNS(svgNS, "tspan");
			tspan.setAttributeNS(null, "dy", i > 0 ? df.format(fontSize * 1.1): "0");
			tspan.setAttributeNS(null, "x", df.format(positionPoint[0]));
			Text tspanText = this.doc.createTextNode(subStrings[i]);
