...
class OCRBoundingBoxOverlay:
    ...
    def overlay_bounding_boxes(
        ...
        if auto_scale_coords and parsed_items:
            ...
            print(
                f"Converting from 1000-bin normalized coordinates to image pixels ({img_width}x{img_height})"
            )
        ...
        # Convert to RGB if saving as JPEG (JPEG doesn't support RGBA or palette modes)
        if output_path.lower().endswith((".jpg", ".jpeg")):
            if image.mode == "RGBA":
                ...
                image = rgb_image
            elif image.mode == "P" or image.mode == "PA":
    ...
    def process_ocr_and_overlay(
        self, image_path: str, ocr_response: Dict[str, Any], output_path: str, **overlay_kwargs
    ) -> List[Dict[str, Any]]:
        """
    ...
    def get_statistics(self, parsed_items: List[Dict[str, Any]]) -> Dict[str, Any]:
        ...
        if parsed_items:
            avg_width = (
                sum((item["bbox"][2] - item["bbox"][0]) for item in parsed_items) / total_items
            )
            avg_height = (
                sum((item["bbox"][3] - item["bbox"][1]) for item in parsed_items) / total_items
            )
        else:
